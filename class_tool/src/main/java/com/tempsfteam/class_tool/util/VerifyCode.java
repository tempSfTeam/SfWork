...
public class VerifyCode {
    ...
    public static boolean doVerifyCode(String code, HttpServletRequest request) {
        final Object vc = request.getSession().getAttribute(VerifyCode.VERIFY_TYPE_COMMENT);
        request.getSession().setAttribute(VerifyCode.VERIFY_TYPE_COMMENT, null);
        if (vc != null && ((String) vc).trim().equalsIgnoreCase(code)) {
            return true;
        }
        return false;
    }
